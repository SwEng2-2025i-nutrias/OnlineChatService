...
class SendMessageUseCase(SendMessage):
    def __init__(self, message_repository: MessageRepository, chat_repository: ChatRepository):
    ...
    def send_message(
        ...
        if not chat:
            raise ValueError("This chat doesn't exist")
        
        if user_id not in [participant["user_id"] for participant in chat.participants]:
            raise ValueError("The user doesn't belong to this chat")
